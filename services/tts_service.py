...
class ElevenLabsTTS(AbstractTTSService):
    ...
    async def generate(self, llm_reply: Dict[str, Any], interaction_count: int):
        ...
        cache_key = f"tts:{partial_response}"
        cached_audio = redis_client.get(cache_key)

        if cached_audio:
            logger.info(f"Using cached TTS response for: {partial_response}")
            await self.emit('speech', partial_response_index, cached_audio.decode('utf-8'), partial_response, interaction_count)
            return
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, params=params, json=data) as response:
                    if response.status == 200:
                        ...
                        # Cache the TTS response in Redis
                        redis_client.set(cache_key, audio_base64)
...
class DeepgramTTS(AbstractTTSService):
    ...
    async def set_voice(self, voice_id):
        ...
        # TODO: Implement voice selection in Deepgram TTS when supported
