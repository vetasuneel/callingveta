...
# Use the pool for creating Redis clients
...
class AbstractLLMService(EventEmitter, ABC):
    ...
    def cache_llm_response(self, key, value):
        redis_client.set(key, json.dumps(value))
        redis_client.expire(key, 3600)  # Cache for 1 hour

    def get_cached_llm_response(self, key):
        cached_data = redis_client.get(key)
        if cached_data:
            return json.loads(cached_data)
        return None

    def cache_user_context(self, call_sid, context):
        redis_client.set(f"user_context:{call_sid}", json.dumps(context))
        redis_client.expire(f"user_context:{call_sid}", 3600)

    def get_cached_user_context(self, call_sid):
        cached_data = redis_client.get(f"user_context:{call_sid}")
        if cached_data:
            return json.loads(cached_data)
        return None
...
class OpenAIService(AbstractLLMService):
    ...
    async def completion(self, text: str, interaction_count: int, role: str = 'user', name: str = 'user'):
        try:
            cache_key = f"openai_response:{text}"
            cached_response = self.get_cached_llm_response(cache_key)

            if cached_response:
                logger.info(f"Using cached response for input: {text}")
                await self.emit_complete_sentences(cached_response, interaction_count)
                return
            ...
            self.cache_llm_response(cache_key, complete_response)
